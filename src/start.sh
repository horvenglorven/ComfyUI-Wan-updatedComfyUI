...
if [ ! -d "$COMFYUI_DIR" ]; then
    echo "First run: seeding ComfyUI from prebuilt image..."
    mv /ComfyUI "$COMFYUI_DIR"
fi

echo "Updating ComfyUI..."
cd "$COMFYUI_DIR" || exit 1
git pull --ff-only || echo "ComfyUI update skipped or needs manual attention."
...
if [ ! -d "$MANAGER_DIR/.git" ]; then
    ...
    git pull --ff-only || echo "ComfyUI-Manager update skipped or needs manual attention."
fi
